...
async def on_message(message):
        ...
        try:
            embed = discord.Embed(
                ...
                description='*Esses são os comandos que não necessitam de permissão.*\n\n\n'
                            ...
                            '**/mineinfo** [username] » Veja o perfil do minecraft de um usuário.\n\n\n'
                            '**UTILITÁRIOS:**\n\n'
    ...
    if message.content.lower().startswith('/head'):
        try:
            ...
            head = discord.Embed(title='', color=COR, description='Head do {}.'.format(nome))
            head.set_image(url=cabeca)
            head.set_author(name=message.server.name, icon_url=message.server.icon_url)
