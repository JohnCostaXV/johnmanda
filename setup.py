    ...
    if message.server is not None:
        ...
        if message.content.lower().startswith('/end'):
            ...
            embed = discord.Embed(
                title="Informações:",
                color=0x03c3f5,
                descripition="**Servidor**:\n**IP**: jogar.end-mc.com\nJogadores online: {}/1500\n\n**Discord**:\n**Foto**: [Download](".format(str(jogadores_online)) + message.server.icon_url + ")\n**ID**: {}\n**Criado em**: {}\n\n**CEOs**: Raaamos, Rosiello_, JohnnCosta, SeveBR e Mystherion.\n\n**Cargos**: {}\n**Emojis**: {}\n**Canais: {}\n\n**Usuários**: {}".format(message.server.id, message.server.created_at.strftime("%d de %beiro de %Y, ás %H:%M"), str(message.server.roles), str(message.server.emojis), str(message.server.channels), str(message.server.members)))
